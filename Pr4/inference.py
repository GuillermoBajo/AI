...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {'Pacman': [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)], 'Ghost0': [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)], 'Ghost1': [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)], 'Observation0': [i for i in range(MAX_NOISE + X_RANGE + Y_RANGE - 1)], 'Observation1': [i for i in range(MAX_NOISE + X_RANGE + Y_RANGE - 1)]}
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Obtener la lista de factores condicionales de probabilidad (CPTs) dados los valores de evidencia.
        Factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Iterar sobre las variables en el orden de eliminación especificado.
        for var in eliminationOrder:
            # Unir los factores actuales por la variable de iteración.
            Factors, joinedFactor = joinFactorsByVariable(Factors, var)
            
            # Verificar si el factor unido tiene más de una variable no condicionada antes de la eliminación.
            if len(joinedFactor.unconditionedVariables()) > 1:
                # Eliminar la variable de iteración del factor unido y obtener un nuevo factor.
                factorNuevo = eliminate(joinedFactor, var)
                
                # Agregar el nuevo factor a la lista de factores actuales.
                Factors.append(factorNuevo)

        # Unir todos los factores restantes en la lista para obtener un único factor final.
        factorFinal = joinFactors(Factors)

        # Normalizar las probabilidades en el factor final y devolverlo como resultado.
        return normalize(factorFinal)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        {}
        """
        # Calcula la suma total de los valores en el diccionario
        total = self.total()

        # Normaliza cada valor dividiéndolo por la suma total
        if total:
            for clave, valor in self.items():
                self[clave] = valor / total
        else:
            exit
    ...
    def sample(self):
        ...
        0.0
        """
        rnd = random.uniform(0, self.total())  # Genera un número aleatorio entre 0 y la suma total de los valores
        x = 0

        # Itera a través de las claves y valores del diccionario
        for clave, valor in self.items():
            # Comprueba si la suma acumulativa supera el número aleatorio
            if x + valor > rnd:
                return clave  # Devuelve la clave correspondiente cuando se supera el número aleatorio
            x += valor  # Actualiza la suma acumulativa para la siguiente iteración
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # Si la posición del fantasma es la posición de la cárcel
            if noisyDistance == None:
                return 1  # Si la distancia ruidosa es None, devuelve probabilidad 1 (el fantasma está en la cárcel)
            else:
                return 0  # Si hay una distancia ruidosa, devuelve probabilidad 0 (el fantasma no está en la cárcel)
        elif noisyDistance == None:
            # Si la distancia ruidosa es None y la posición del fantasma no es la cárcel, devuelve probabilidad 0
            return 0
        dist = manhattanDistance(pacmanPosition, ghostPosition)

        # Calcula la probabilidad de observación usando la función getObservationProbability
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Obtener las creencias actuales del agente
        selfbelieves = self.beliefs

        # Obtener la posición de la cárcel y la posición actual de Pacman en el estado del juego
        jailPos = self.getJailPosition()
        pacPos = gameState.getPacmanPosition()

        # Iterar sobre todas las posiciones posibles en el juego
        for pos in self.allPositions:
            # Calcular la probabilidad de observar la evidencia (observation) dada la posición actual de Pacman y la posición iterada
            probs = self.getObservationProb(observation, pacPos, pos, jailPos)

            # Actualizar la creencia para la posición iterada multiplicándola por la probabilidad calculada
            selfbelieves[pos] *= probs  # 3. Actualización de la creencia para la posición iterada
        self.beliefs.normalize()
